...
class myQueue: #스택을 이용하여 큐 클래스를 정의 (스택은 리스트를 이용함)
	...
	def enqueue(self, val): #enq 입력이 들어올 때
		self.items.append(val) #리스트에 append -> 시간 복잡도 : O(1) (append 연산이기 때문에)
	...
	def dequeue(self): #deq 입력이 들어올 때
		if self.front_index == len(self.items): #리스트의 길이가 0일때, 즉 리스트가 비었을 때
			...
			x = self.items[self.front_index] #index로 원소에 바로 접근하는 방식은 시간복잡도가 O(1) 
			self.front_index += 1 #index를 하나 뒤로 이동 -> 다음 deq 입력이 들어왔을때 O(1)의 시간에 동작할 수 있게 함
